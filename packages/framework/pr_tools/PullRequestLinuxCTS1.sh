...
sbatch --output=${stdout} --error=${stderr} --wait --account=fy200165 --exclusive --partition=short --ntasks=16 --job-name="${JOB_NAME}" --time="${SLURM_CTEST_TIME_LIMIT}" "TMPDIR=/dev/shm/trilinos mkdir ${TMPDIR} && \"${WORKSPACE}\"/Trilinos/packages/framework/pr_tools/PullRequestLinuxDriver.sh --on_cts1"
ESTAT=$?
